...
class SiteEventsChartsHolderIndividual extends React.Component {
  constructor() {
    super();
  ...
  componentDidMount() {
    ...
    if (this.props.site_ids.length) {
      request('/api/sighting/bysite?site_id=&' + this.props.site_ids.map(x=> 'site_id=' + x).join('&'), {
        method: 'GET',
      }).then((res) => {
...
export default SiteEventsChartsHolderIndividual;
