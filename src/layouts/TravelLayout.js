...
class TravelLayout extends React.PureComponent {
        ...
        constructor(props) {
            ...
            this.menus = props.travelnavData.reduce((arr, current) => arr.concat(current.children), []);
            this.state = {
                openKeys: this.getDefaultCollapsedSubMenus(props),
            };
        }
        getChildContext() {
            const { location, travelnavData, getRouteData } = this.props;
            const routeData = getRouteData('TravelLayout');
            const firstMenuData = travelnavData.reduce((arr, current) => arr.concat(current.children), []);
            ...
            return menusData.map((item) => {
                ...
                return ( <
                    Menu.Item key = { item.key || item.path } > {
                        /^https?:\/\//.test(itemPath) ? ( <
                            a href = { itemPath }
                            target = { item.target } > { icon } < span > { item.name } < /span> <
                            /a>
                        ) : ( <
                            Link to = { itemPath }
                            target = { item.target }
        ...
        getPageTitle() {
            ...
            let title = 'LUCA';
            getRouteData('TravelLayout').forEach((item) => {
        ...
        getNoticeData() {
            ...
            const newNotices = notices.map((notice) => {
                ...
                if (newNotice.extra && newNotice.status) {
                    ...
                    newNotice.extra = < Tag color = { color }
                    style = {
                        { marginRight: 0 } } > { newNotice.extra } < /Tag>;
                }
                return newNotice;
            });
        ...
        render() {
            ...
            const layout = ( <
                ...
                Content style = {
                    { margin: '0px 0px 0', height: '100%' } } >
                <
                div style = {
                    { minHeight: 'calc(100vh - 260px)' } } >
                <
                Switch > {
                    getRouteData('TravelLayout').map(item =>
                        ( <
